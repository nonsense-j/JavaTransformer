...
/**
 * Transform that transfers local variables to static global fields.
 * This transformation moves literal values from local scope to static class-level fields.
 */
public class TransferLocalVarToStaticGlobal extends Transform {
    ...
    public List<ASTNode> check(TypeWrapper wrapper, ASTNode srcNode) {
        ...
        if(LoopStatement.isLoopStatement(srcNode)) {
            ...
            if(loop.getBody() instanceof Block) {
                @SuppressWarnings("unchecked")
                List<Statement> statements = ((Block) loop.getBody()).statements();
                subNodes = TypeWrapper.getChildrenNodes(new ArrayList<ASTNode>(statements));
            } else {
    ...
    public String getDescription() {
        return "Transfers local variables to static global fields";
    }
}