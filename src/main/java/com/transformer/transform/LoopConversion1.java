...
/**
 * Transformation that converts traditional for loops and enhanced for loops to forEach method calls.
 */
public class LoopConversion1 extends Transform {

    private static LoopConversion1 instance = new LoopConversion1();
                    ...
                    if (initializer instanceof MethodInvocation) {
                        if (((MethodInvocation) rightOperand).getExpression() instanceof SimpleName && ((MethodInvocation) initializer).getExpression() instanceof SimpleName) {
                            ...
                            if (getInvoker.equals(sizeInvoker) && "get".equals(get)) {
                                ...
                                newExpression.setExpression(ast.newSimpleName(getInvoker)); // list in list.forEach(item)
                                newExpression.setName(ast.newSimpleName("forEach"));
                                LambdaExpression lambdaExpression = ast.newLambdaExpression();
                                VariableDeclarationFragment fragment = ast.newVariableDeclarationFragment();
                        ...
                        if (method != null && method.getBody() != null) {
                            for (int i = 0; i < method.getBody().statements().size(); i++) {
                                ...
                                if (statement instanceof EnhancedForStatement) {
                                    if (TypeWrapper.compareNode(statement, node)) {
                                        break;
                                    }
                                }
                            }
    ...
    public String getDescription() {
        return "Converts traditional for loops and enhanced for loops to forEach method calls";
    }
}