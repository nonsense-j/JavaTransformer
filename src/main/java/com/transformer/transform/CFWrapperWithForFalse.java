...
/**
 * Transform that wraps statements with a for loop that has a false condition.
 * This creates unreachable code wrapped in a for loop.
 */
public class CFWrapperWithForFalse extends Transform {
    ...
    public boolean run(ASTNode targetNode, TypeWrapper wrapper, ASTNode brother, ASTNode srcNode) {
        ...
        List<Expression> initializers = newForStatement.initializers();
        initializers.add(newVdExpression);
        ...
        @SuppressWarnings("unchecked")
        List<Expression> updaters = newForStatement.updaters();
        updaters.add(postfixExpression);

        Block newBodyBlock = ast.newBlock();
        Statement newStatement = (Statement) ASTNode.copySubtree(ast, srcNode);
        
        @SuppressWarnings("unchecked")
        List<Statement> bodyStatements = newBodyBlock.statements();
        ...
        if(methodStatements.contains(srcNode)) {
            ...
            @SuppressWarnings("unchecked")
            List<Statement> newBlockStatements = newBlock.statements();
            newBlockStatements.add(newForStatement);
            newBlockStatements.add((Statement) ASTNode.copySubtree(ast, srcNode));
    ...
    public String getDescription() {
        return "Wraps statements with a for loop that has a false condition";
    }
}