...
/**
 * Transform that adds argument assignment by extracting literals into local variables.
 * This transformation replaces literal values with variable assignments.
 */
public class AddArgAssignment extends Transform {
    ...
    public boolean run(ASTNode targetNode, TypeWrapper wrapper, ASTNode brotherStatement, ASTNode srcNode) {
        ...
        List<Modifier> modifiers = newVdStatement.modifiers();
        modifiers.add(ast.newModifier(Modifier.ModifierKeyword.FINAL_KEYWORD));
        
        // Set appropriate type based on literal type
    ...
    public List<ASTNode> check(TypeWrapper wrapper, ASTNode node) {
        ...
        if(node instanceof ReturnStatement) {
            if(TypeWrapper.isLiteral(((ReturnStatement) node).getExpression())) {
                return nodes;
            }
            for(ASTNode subNode : subNodes) {
                if(TypeWrapper.isLiteral(subNode)) {
    ...
    public String getDescription() {
        return "Adds argument assignment by extracting literals into local variables";
    }
}